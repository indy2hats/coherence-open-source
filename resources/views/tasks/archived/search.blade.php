...
<div class="ibox-content mb10 panel filter-area">
    <form action="{{ route('archivedTaskSearch') }}" method="post" autocomplete="off" id="search-task">
        @csrf
...
>                    @endif
            ...
            <div class="col-sm-6 col-md-3">
                <select class="chosen-select" id="search_project_name" name="search_project_name">
                    ...
                    <option value="{{$searchedProject->id}}" selected>{{$searchedProject->project_name}}</option>
                    @else
        ...
        <div class="row">
            ...
            <div class="col-sm-6 col-md-3">
                <a href="/archived-tasks" class="btn btn-w-m btn-info" style="padding: 4px 12px;"> Clear Filters</a>
            </div>
        </div>
    </form>
</div>
