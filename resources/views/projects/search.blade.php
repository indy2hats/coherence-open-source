...
<div class="ibox-content panel mb10">
	<form action="{{route('project_search')}}" method="post" autocomplete="off" id="search-project">
	@csrf
	<div class="row filter-row" style="padding-top: 10px;padding-bottom: 10px">
		...
		<div class="col-sm-6 col-md-2">
			<select class="chosen-select" id="search_technology" name="projectTechnology">
				<option value="">Select Technology</option>
				@foreach ($technologies as $technology)
				<option value="{{$technology->id}}" {{request()->projectTechnology == $technology->id ? 'selected' : ''}}>{{$technology->name}}</option>
				@endforeach
			</select>
		</div>
		<div class="col-sm-6 col-md-2">
