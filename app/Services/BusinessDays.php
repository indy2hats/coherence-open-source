...
class BusinessDays
{
    ...
    public function addHolidays(array $dates): self
    {
        foreach ($dates as $date) {
            $this->addHoliday(Carbon::parse($date));
        }
    ...
    {
        return array_search($day->dayOfWeek, $this->weekendDays) !== false;
    }
    ...
    {
        $days = $from->diffInDaysFiltered(function (Carbon $day) {
            return $this->isOpenedDay($day);
        }, $to);

        if ($this->isOpenedDay($to)) {
            $days = $days + 1;
        }
     ...
     * @return Carbon
     */
    public function getLastWorkingDay($date): Carbon
    {
        if (! $this->isOpenedDay($date)) {
            $date = $date->subDay();

            return $this->getLastWorkingDay($date);
        }
